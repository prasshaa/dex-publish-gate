...
*Important caveat:* this number is moved by two different things — the actual quality/riskiness of what's being submitted (upstream, not something the gate controls), and how well-calibrated the gate itself is (something it does control). On its own, a drop in this metric can't distinguish "the gate got worse" from "content got riskier this month." It should always be read alongside the gate's own precision/recall against a fixed labeled golden set (from the offline eval process) — if the golden-set numbers hold steady while the live rate drops, the cause is upstream, not the gate.
